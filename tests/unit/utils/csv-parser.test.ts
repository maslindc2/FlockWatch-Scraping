...
describe("Test CSVParser class", () => {
    ...
    it("should return an array containing objects and their associated values when a csv string is provided for the fileContent", () => {
        ...
        const res = CSVParser.parseCSV(csvString, delimiter, startFromRow, columns);
    ...
    it("should throw an error when startFromRow is out of bounds", () => {
        ...
        const delimiter = ","
        const startFromRow = 1
        expect(() => CSVParser.parseCSV(malformedCSV, delimiter, startFromRow, false)).toThrow(new Error("CSV is malformed resulted in Error: Invalid Record Length: expect 2, got 1 on line 3"))
    });
});