...
class USDAScrapingService {
     ...
     */
    private async setupBrowser(config: USDAScrapingConfig): Promise<{browser: Browser, page: Page}> {
    ...
    private async selectTimePeriod(): Promise<void> {
        ...
        await this.page.getByRole('menuitem', { name: 'Total Outbreak' }).click();
    }
    ...
    private async selectDownloadOptions(): Promise<void> {
        ...
        await this.page.locator('[role="button"]:has-text("Download Data")').click();
        ...
        await this.page.getByTestId("crosstab-options-dialog-radio-csv-Label").click();
    }
    ...
    private async initiateDownload(): Promise<string> {
        ...
        const responsePromise = this.page.waitForResponse(response => response.url().includes("tempfile"));
        
        // Start the download by clicking the download button
        await this.page.getByTestId("export-crosstab-export-Button").filter({hasText: "Download"}).click();
    ...
    public async getFlockCasesFromUSDA(): Promise<SharedArrayBuffer>{
        try{
            ...
            const {browser:browserInstance, page} = await this.setupBrowser(this.config);
            ...
            const response = await axios.get<SharedArrayBuffer>(downloadURL, {responseType: "arraybuffer"}); 
            const csvData = response.data;

            logger.info(`Successfully downloaded CSV with Axios (${csvData.byteLength} bytes)`);
            
            // Return the data
            return csvData;
        }catch (error) {
            logger.error(`Failed to scrape USDA data: ${error instanceof Error ? error.message : 'Unknown error'}`);
            throw new Error(`Failed to scrape USDA data: ${error instanceof Error ? error.message : 'Unknown error'}`);
        } finally {
            if(this.browser){
